...
class PharmacyConfirmSummaryWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InvoiceSummaryCard(
      ...
      Visibility(
        visible: (order.loyaltyPoints) > 0,
        child: BuildSummaryHeader(
          title: tr("pointsDiscount"),
          details: order.loyaltyPointsValue,
      ...
      Visibility(
        visible: order.couponDiscount.isNotEmpty &&  (double.tryParse(order.couponDiscount) ?? 0) > 0,
        child: BuildSummaryHeader(
          title: tr("voucherDiscount"),
          details: order.couponDiscount,
          detailsColor: context.colors.primary,
          useDirhamPrice: true,
          isDiscount: true,
        ),
      ...
      if (!context.isShareHolder)
        Column(
          ...
          children: [
            ...
            if ((order.loyaltyPoints ) > 0) Gaps.vGap8,
            NewPointsBalanceWidget(
              cubit: controller.loyaltyPointsBalanceBloc,
              gainedPoints: order.expectedLoyaltyPoints,
            ),
          ],
        )
    ]);
