...
class ShareHolderAppBarWidget extends StatelessWidget
  ...
  final String email;
  const ShareHolderAppBarWidget(
      {super.key, required this.controller, required this.email});
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.only(
          right: 20,
          left: 20,
          top: Platform.isIOS ? kToolbarHeight : kToolbarHeight - 30,
          bottom: 6),
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    tr("shareholders"),
                    style:
                        const AppTextStyle.s12_w500(color: Color(0xff7A4702)),
                  ),
                )
              ],
            ),
  ...
  Text _buildText(BuildContext context) {
    var name = context
        .select<UserCubit, String>((value) => value.state.model?.name ?? "");
    return Text(
      "${tr("welcome")} $name",
      maxLines: 2,
