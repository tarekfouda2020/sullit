...
class RestaurantCartController {
  ...
  final GenericBloc<Shop?> restaurantBloc = GenericBloc<Shop?>(null);
  ...
  RestaurantCartController({this.restaurantId, this.fromRestaurantDetails = true, this.preSelectedBranchId});

  Future<void> getData() async {
    getCartItems(refresh: false);
    getCartItems();
    _getRestaurantDetails(fromRemote: false);
  ...
  Future<void> clearCart(BuildContext context) async {
    ...
    final params =
        ClearCartParams(macAddress: token ?? "", type: CartTypeEnum.restaurant);
  ...
  Future<void> _getRestaurantDetails({bool fromRemote = true}) async {
    var data = await GetShopDetails().call(
      ShopIdParams(shopId: restaurantId!, refresh: fromRemote),
    );
    if(data!= null){
      restaurantBloc.onUpdateData(data);
    }
