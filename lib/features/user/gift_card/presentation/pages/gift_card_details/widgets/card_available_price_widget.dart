...
class CardAvailablePrice extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: BlocBuilder<GenericBloc<GiftCardDomainModel?>,
        ...
        builder: (context, state) {
          if (state is GenericUpdateState) {
            return Row(
              ...
              children: [
                Text(tr("remainAmount"),
                    style: AppTextStyle.s22_w400(color: context.colors.black)),
                Gaps.hGap5,
                DirhamPrice(
                  amount: state.data!.price,
                  textStyle:
                      AppTextStyle.s24_w700(color: context.colors.primary),
                ),
              ],
            );
          } else {
