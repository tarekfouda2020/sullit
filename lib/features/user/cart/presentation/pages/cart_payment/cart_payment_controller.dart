...
class CartPaymentController {
  ...
  CartCheckOutSavedData get _pageSavedData =>
      getIt<CartNavigateHelper>().cartCheckOutPageData;
  ...
  void calculateDiscount() {
    String subTotal =
        shippingBloc.state.data!.summary.subTotal.replaceAll("", "").replaceAll(",", "");
  ...
  Future<void> applyGiftCard(BuildContext context) async {
    ...
    if (giftCardFormKey.currentState!.validate()) {
      ...
      await ApplyGiftCard()
          .call(ApplyGiftCardParams(giftCardCode: giftCardCode.text))
          .then((value) {
        if (value != null) {
          isGiftCardApplied = true;
          shippingBloc.state.data!.summary = value.summary;
          shippingBloc.state.data!.summary.appliedGiftCard =
  ...
  Future<void> getInstructions({bool fromRemote = true}) async {
    ...
    List<DeliveryInstructionModel> savedInstructions =
        getIt<CartNavigateHelper>()
                .cartCheckOutPageData
                .selectedDriverInstructions ??
            [];
  ...
  void selectInstructions(DeliveryInstructionModel model) {
    ...
    getIt<CartNavigateHelper>().updateDriverInstructions(
        instruction: _selectedInstructions(), driverNotes: driverNotesCtr.text);
  ...
  void whileEnterPickerNotes() {
    getIt<CartNavigateHelper>()
        .updatePickerNotes(driverNotes: pickerNotesCtr.text);
  }
  ...
  Future<void> getRemoteData() async {
    ...
    if (data != null) {
      double oldSubTotal = double.parse(
          _pageSavedData.orderSummaryCheckOut?.summary.subTotal.cleanNumber() ??
              "0.0");
