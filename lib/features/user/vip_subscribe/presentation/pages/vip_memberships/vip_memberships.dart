...
class _VipMemberShipsState extends State<VipMemberShips> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocBuilder<GenericBloc<VipCurrentPlanDomainModel?>,
            ...
            return ListView(
              ...
              children: [
                if (state.data!.currentSubscription != null)
                  CurrentSubscriptionSectionWidget(
                      controller: controller,
                      model: state.data!.currentSubscription!)
                else
                ...
                Gaps.vGap10,
                ...List.generate(state.data!.otherSubscriptions.length,
                    (index) {
                  return Visibility(
                    visible:
                        state.data!.otherSubscriptions[index].byInvite == false,
                    child: MembershipItemWidget(
                      model: state.data!.otherSubscriptions[index],
                      isBottomSheet: false,
                      onSelect: () => controller.selectMembership(
                          state.data!.otherSubscriptions[index]),
                    ),
                  );
                }),
