...
class LocationIqHelper {
  ...
  Future<Either<Failure, List<LocationIQPlace>>> getAutoCompleteLocations(
      String keyword,
      {bool refresh = true}) async {
    try {
      LocationIqParams params = _autoCompleteParams(keyword);
      final String url =
          '$_locationIqBaseUrl${ApiNames.locationAutoComplete}${params.autoCompleteUrl()}';
  ...
  Future<CustomAddressModel?> getFullAddress(LatLng latLng,
    ...
    try {
      ...
      final url =
          '$_locationIqBaseUrl${ApiNames.locationAddress}${params.addressUrl()}';
      // final queryParams = {
      //   'key': _apiKey,
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        final address = CustomAddressModel(
          countryName: setCountryName ? addressData['country'] as String? : "",
